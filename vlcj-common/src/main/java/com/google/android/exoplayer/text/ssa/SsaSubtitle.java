...
/* package */ final class SsaSubtitle implements Subtitle {
	...
	/**
	 * @param cues
	 *            The cues in the subtitle. Null entries may be used to
	 *            represent empty cues.
	 * @param cueTimesUs
	...
	public long getEventTime(int index) {
		assert (index >= 0);
		assert (index < cueTimesUs.length);
	...
	public List<Cue> getCues(long timeUs) {
		...
		if (index == -1 || cues[index] == null) {
			// timeUs is earlier than the start of the first cue, or we have an
			// empty cue.
